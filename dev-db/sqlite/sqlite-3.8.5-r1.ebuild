...
RESTRICT="fetch"
DESCRIPTION="A SQL Database Engine in a C Library with SEE"
HOMEPAGE="http://sqlite.org/see"
SRC_URI="sqlitesee? ( http://sqlite.org/2014/see-sources.zip )
	http://sqlite.org/2014/${PN}-autoconf-${SRC_PV}.tar.gz
...
IUSE="debug icu +readline secure-delete static-libs tcl test +sqlitesee"
...
pkg_nofetch() {
	...
	einfo "  - see-sources.zip"
	SPKG="http://sqlite.org/2014/${PN}-autoconf-${SRC_PV}.tar.gz"
	einfo "  - ${SPKG}"
...
#	fi
	...
	einfo "Adding AES256 OFB support..."
	cat "${WORKDIR}/see-sources/see-aes256-ofb.c" >> "${WORKDIR}/${PN}-autoconf-${SRC_PV}/sqlite3.c"
	cp "${WORKDIR}/see-sources/shell.c" "${WORKDIR}/${PN}-autoconf-${SRC_PV}/shell.c"
...
multilib_src_configure() {
	...
	# SEE Support
	append-cppflags -DSQLITE_HAS_CODEC=1
